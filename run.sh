...
if [ -n "$WERCKER_SLACK_NOTIFIER_PASSED_MSG" ]; then
  export MESSAGE="$WERCKER_SLACK_NOTIFIER_PASSED_MSG"
...
if [ "$WERCKER_RESULT" = "failed" ]; then
  if [ -n "$WERCKER_SLACK_NOTIFIER_FAILED_MSG" ]; then
    export MESSAGE="$WERCKER_SLACK_NOTIFIER_FAILED_MSG"
    export FALLBACK="$MESSAGE"
  else
    # TODO: what if passed_msg is set but failed_msg is not?
